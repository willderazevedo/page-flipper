...
function wa_page_flipper_background_meta_box_save( $post_id ) {
    ...
    if (
        ! isset( $_POST['wa_page_flipper_background_nonce'] ) ||
        ! wp_verify_nonce(
            sanitize_text_field(
                wp_unslash( $_POST['wa_page_flipper_background_nonce'] )
            ),
            'wa_page_flipper_background_nonce_action'
        )
    ) {
        return;
    }
